...
def process_database():
    with open(r"fma_test.txt", "r") as f:
        lines = f.readlines()
    ...
    for line in lines:
        # try:
        input_wav = line.strip()
        feature = fingerprinting([input_wav])[0]
        features.append(feature.detach().cpu().numpy())
        indicies.append(int(os.path.basename(input_wav).split(".")[0]))
        # except:
            # print(f"Error processing {input_wav}")
    np.save(r"test_features.npy", np.array(features))
    np.save(r"test_indicies.npy", np.array(indicies))



def load_database():
    features = torch.from_numpy(np.load(r"test_features.npy")).to('cuda')
    indicies = torch.from_numpy(np.load(r"test_indicies.npy")).to('cuda')
...
def top_k(features, indicies, query, k=10):
    ...
    score, indices = torch.topk(res, k=k, dim=0)
    return indicies[indices]
...
def fingerprinting(inputs):
    ...
    """
    model = CQTNet()
    model.load_state_dict(torch.load(r"CQTNet.pth"))
    model.eval()
    model.to('cuda')
    features = []
    for data in inputs:
        data = CQT(data, out_path=None)
        feature = get_feature(model, data)
...
def lookup(query, k=10):
    ...
    results = []
    for q in query:
        results.append(top_k(features, indicies, q, k=k))
    return results
...
if __name__ == '__main__':
    # process_database()
    # load_database()
    print(lookup([fingerprinting([r"E:\cs682\data\fma_small_wav\038896.wav"])]))
